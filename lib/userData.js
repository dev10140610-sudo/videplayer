...
// --- Заметки (отдельная коллекция, не пересекается с историей) ---
// Каждая заметка = снимок как в истории (поле `id` = id сериала, чтобы payload
// resume совпадал с историей) + свой заголовок `title`. Несколько заметок на сериал.
const notesCol = (uid) => collection(db, 'users', uid, 'notes');

const NOTES_LIMIT = 50;

// Создание заметки (авто-id, историю не трогает).
export const saveNote = async (uid, note) => {
  await addDoc(notesCol(uid), { ...note, updatedAt: serverTimestamp() });
};

export const listNotes = async (uid, n = NOTES_LIMIT) => {
  const snap = await getDocs(query(notesCol(uid), orderBy('updatedAt', 'desc'), limit(n)));
  return snap.docs.map((d) => ({ noteId: d.id, ...d.data() }));
};

// Данные заметки по её noteId (или null) — используются как payload для resume.
export const loadNote = async (uid, noteId) => {
  const snap = await getDoc(doc(notesCol(uid), String(noteId)));
  return snap.exists() ? snap.data() : null;
};

export const removeNote = async (uid, noteId) => {
  await deleteDoc(doc(notesCol(uid), String(noteId)));
};
